    ...
    <form method="POST" action="{{ route('comments.store') }}">
      ...
      <input type="hidden" name="post_id" value="{{ $post_id }}" />
        <div class="row clearfix">
            ...
            @else
                <input type="hidden" name="comment_author" placeholder="Votre nom" value="{{ Auth::user()->name }} {{ Auth::user()->surname }}">
                <input type="hidden" name="comment_author_email" placeholder="Votre Email" value="{{ Auth::user()->email }}" disabled>
            @endif
